...
export function renderETAPage(view: ETAView): string {
  ...
  <main class="eta-page">
    ...
    <section class="cover" aria-live="polite">
      ...
      <p class="notice" id="notice">${escapeHTML(result?.stale ? '資料可能延遲，請留意站牌資訊' : result?.source === 'schedule' ? '資料為時刻表推估，實際到站時間可能不同' : error ?? '')}</p>
    </section>
  ...
  <script type="module">
    import { activeBoardId, migrateBoards, setActiveCity, syncActiveBoard, writeBoards } from '/assets/boards.js';
    ...
    async function refreshBoard() {
      ...
      writeBoards(migrateBoards(initialBoard).map(board => board.id === currentBoard.id ? currentBoard : board));
      ...
      else if (fresh.some(item => item.source === 'schedule')) noticeNode.textContent = '部分資料為時刻表推估，實際到站時間可能不同';
    ...
    if (useLocalBoard) {
      const storedBoards = migrateBoards(initialBoard);
...
function setupScript(): string {
  ...
  import {activeBoardId,busKey,clearLocalData,migrateBoards,newBoardId,setActiveBoard,syncActiveBoard,writeBoards} from '/assets/boards.js';
  ...
  document.querySelector('#clear-local-button').onclick=()=>{if(confirm('確定清除所有本機資料?常用站牌與封面設定會全部刪除,無法復原。')){clearLocalData();renderBoards()}};
