...
export function renderETAPage(view: ETAView): string {
  const { query, result, error, notice, useLocalBoard } = view
  const initialBoard = {
    version: 2,
    id: 'default',
  ...
  <script type="module">
    ...
    const tdxWarningMessages = ${safeJSON(tdxWarningMessages)};
    ...
    <section class="panel">
      ...
      <details class="advanced-panel">
        ...
        <div class="advanced-section">
          ...
          <div class="credential-field"><label for="tdx-client-id">Client ID</label><input id="tdx-client-id" autocomplete="off" spellcheck="false"></div>
          <div class="credential-field"><label for="tdx-client-secret">Client Secret</label><input id="tdx-client-secret" placeholder="Client Secret" type="password" autocomplete="off"></div>
...
function setupScript(): string {
  ...
  let routes=[],category='全部',selectedRoute=null,requestId=0;
  ...
  async function loadRoutes(){grid.replaceChildren();message.textContent='正在載入路線…';directionStep.hidden=true;suggestionStep.hidden=true;const id=++requestId;try{const r=await fetch('/api/v1/routes?schema=2&city='+encodeURIComponent(city.value),{cache:'no-store',headers:tdxHeaders()});const body=await r.json();if(!r.ok)throw Error(body.error);if(id!==requestId)return;routes=body.routes;message.textContent='共 '+routes.length+' 條路線';renderCategories();renderRoutes()}catch(e){if(id!==requestId)return;message.textContent=e.message||'路線載入失敗'}}
  async function chooseRoute(route){selectedRoute=route;message.textContent='正在載入 '+route.routeName+' 的站牌…';directionStep.hidden=true;suggestionStep.hidden=true;const id=++requestId;const p=new URLSearchParams({city:city.value,route:route.routeName});if(route.routeUid)p.set('routeUid',route.routeUid);try{const r=await fetch('/api/v1/stops?'+p,{headers:tdxHeaders()}),body=await r.json();if(!r.ok)throw Error(body.error);if(id!==requestId)return;routePicker.hidden=true;renderDirections(body.groups)}catch(e){if(id!==requestId)return;message.textContent=e.message||'站牌載入失敗'}}
  ...
  tdxSave.onclick=async()=>{const current=getTdxAuthState().auth,clientId=tdxId.value.trim(),typedSecret=tdxSecret.value.trim(),clientSecret=typedSecret||(current&&current.clientId===clientId?current.clientSecret:'');if(!clientId||!clientSecret){tdxMessage.textContent='Client ID 與 Client Secret 都要填';return}tdxSave.disabled=true;tdxMessage.textContent='正在跟 TDX 打聲招呼…';try{const r=await fetch('/api/v1/tdx/verify',{cache:'no-store',headers:{'x-tdx-client-id':clientId,'x-tdx-client-secret':clientSecret}});const body=await r.json();if(!r.ok)throw Error(body.error);const persistence=tdxRemember.checked?'device':'session';setTdxAuth({clientId,clientSecret},persistence);tdxSecret.value='';renderTdx(persistence==='device'?'憑證有效，已記住於此裝置。':'憑證有效，只保留在此分頁；關閉分頁後即移除。')}catch(e){tdxMessage.textContent=e.message||'驗證失敗，稍後再試'}tdxSave.disabled=false};
  tdxRemove.onclick=()=>{clearTdxAuth();tdxId.value='';tdxSecret.value='';tdxRemember.checked=false;renderTdx('已移除，回到共用額度。')};
  getTdxAuthState();renderTdx(consumeTdxAuthMigrationNotice()?'舊版長期保存的憑證已改為只保留在此分頁；若要繼續跨次使用，請勾選「記住於此裝置」後重新儲存。':undefined);
  document.querySelector('#clear-local-button').onclick=()=>{if(confirm('確定清除所有本機資料?常用站牌、封面設定與 TDX 憑證會全部刪除,無法復原。')){clearLocalData();tdxId.value='';tdxSecret.value='';tdxRemember.checked=false;renderBoards();renderTdx()}};
...
function pageShell(title: string, body: string, script = '', description = siteSearchDescription): string {
  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#f7f2e8"><meta name="description" content="${escapeHTML(description)}"><meta property="og:title" content="${escapeHTML(title)}"><meta property="og:description" content="${escapeHTML(siteSocialDescription)}"><meta property="og:site_name" content="Mochi Bus">${renderWebsiteStructuredData()}<link rel="manifest" href="/manifest.webmanifest"><link rel="icon" href="/icon.svg" type="image/svg+xml"><link rel="icon" href="/favicon.ico" sizes="any"><link rel="apple-touch-icon" href="/apple-touch-icon.png"><title>${escapeHTML(title)}</title><style>${styles}${enhancementStyles}${credentialStyles}</style></head><body>${body}${script}</body></html>`
}
...
const credentialStyles = `.credential-field{display:grid;gap:6px}.credential-field>label{color:#716a60;font-size:13px;font-weight:750}.tdx-remember{display:flex;align-items:flex-start;gap:10px;padding:11px;border:1px solid #ded6c9;border-radius:12px;cursor:pointer}.tdx-remember input{width:auto;margin-top:3px}.tdx-remember span{display:grid;gap:3px}.tdx-remember small{color:#777066;line-height:1.5}@media(prefers-color-scheme:dark){.credential-field>label,.tdx-remember small{color:#aaa197}.tdx-remember{border-color:#4d473e}}`
