...
export async function getStopPlaceRoutes(env: TransitBindings, city: string, placeId: string) {
  ...
  const result = await env.TRANSIT_DB.prepare(`
    SELECT DISTINCT r.route_name, p.pattern_id, p.direction,
...
export async function getOneTransferRoutes(
  ...
  const result = await env.TRANSIT_DB.prepare(`
    ...
    SELECT DISTINCT
      first_legs.transfer_place_id, place.place_name AS transfer_name,
    ...
    FROM first_legs
    JOIN second_legs ON second_legs.transfer_place_id = first_legs.transfer_place_id
    ...
    JOIN stop_places place
      ON place.version = first_pattern.version AND place.place_id = first_legs.transfer_place_id
  ...
  return [...new Map(plans.map((plan) => [
    `${plan.first.routeName}:${plan.second.routeName}:${plan.transferPlaceId}`,
    plan,
  ])).values()]
