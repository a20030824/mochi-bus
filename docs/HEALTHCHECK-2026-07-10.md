...
| ARCH-001 | P2 | 大量 browser JS 內嵌字串未被完整 typecheck/lint，地圖主檔過大 | `web/map/main.ts`、`web/map/map-api-client.ts`、`web/map/city-network-controller.ts`、`web/map/timetable-view.ts` | Phase 5 | In Progress：setup 已模組化；map transport、全路網 controller、時刻表 view 已拆出，主檔由約 3100 行降至約 2500 行；行程規劃與路線 detail 邊界仍待後續拆分 |
...
| PIPE-001 | P2 | token fetch 無 timeout/retry，Retry-After 解析有陷阱，單城失敗中止整批 | `scripts/sync-transit-snapshot.mjs:22-54`、`.github/workflows/sync-transit.yml:72-74` | Phase 3 | Verified：timeout/retry 共用邏輯已上線並以手動 dispatch（run `29197805829`）完成一次 production publish 全閘門實跑，發布 `20260712T151455398Z`；retry 分支尚未在真實 TDX 故障下觀察，細節見 Phase 3 下方 2026-07-12 紀錄 |
...
> 2026-07-12 PIPE-001 production 實跑結果：修正以 commit `1577074`（改名為 `scripts/sync-transit-snapshot.mjs`，rename 見 `4151288`）推上 main 並通過 CI 後，手動 `gh workflow run sync-transit.yml -f city=Chiayi`（run `29197805829`）以新程式碼走完 generate → validate → publish → smoke 全閘門，2m26s 成功：發布 Chiayi `20260712T151455398Z`（66 routes、273 patterns、3,106 stops、1,170 places、11,844 pattern-stop references），previous `20260711T230007001Z` 保留可回滾；發布後公開 routes API 冒煙 200。本次 TDX 全程健康，retry／timeout 分支沒有機會實際觸發——能證明的是新共用邏輯在 production pipeline 的 happy path 完整可用、token fetch 改走 `fetchWithRetry` 不影響正常流程；`Retry-After` 缺席分支與重試行為的證據仍來自程式碼路徑推導。考量原風險（token fetch 無 timeout/retry、`Number(null)` 陷阱、單城失敗隔離）的三個面向都已有對應修正且 pipeline 實跑通過，PIPE-001 標記 Verified；若未來 TDX 實際故障時發現重試行為不符預期，依更新規則另開新 ID 追蹤。
...
> 2026-07-12 部署與線上驗證結果：工作區累積的六個 commit（`4151288` 腳本改名、`1577074` PIPE-001 timeout/retry、`89ef37b` `resolveTDXNotice()` Object.hasOwn 防護、`1367c5e` SEO-001 canonical/og:url、`3c06382` A11Y-001 drawer retry 與 BYOK 錯誤關聯、`237ed16` SECURITY.md/CONTRIBUTING.md/WebP hero/文件）已推上 main，CI 綠燈（run `29197563352`，29s），並部署為 `1562fe61-f961-4e74-b667-e5d3f6308267`（100%）；前一版 `f6ab4862-e113-4f63-a6bd-27b85f5c5a70` 可直接回滾——注意這個前一版是 2026-07-12 06:18Z 出現的部署，未見於本文件既有紀錄（06:59Z 冒煙測試時首頁尚無 canonical，可確認它不含本輪變更）。線上驗證：`/`、`/setup`、`/map?city=Chiayi` 的 canonical 與 og:url 均已標準化為 `https://bus.moc96336.com` + 實際 path/query；`/setup` 的 noindex 保留；部署後的 `map.js` 含「再試一次」retry 邏輯、setup 頁 HTML 兩個憑證欄位都有 `aria-describedby="tdx-message"`、`setup.js` 含 `aria-invalid` 切換；HSTS/CSP/`X-Frame-Options` 正常；cities（0.21s）、Chiayi routes（0.34s）、nearby（0.51s）皆 200。A11Y-001 的 skeleton retry 與 BYOK 錯誤關聯、SEO-001 的 canonical/og:url 至此完成 production 部署與線上確認；兩項風險 ID 的剩餘 Open 項目不變（A11Y-001：品牌色對比待設計覆核、全站 axe、focus-visible 盤點；SEO-001：1200×630 banner、sitemap 邊界）。同日 npm audit 0 vulnerabilities、TLS 1.0/1.1 拒絕／1.2/1.3 正常、HTTP 301 跳轉均複測通過；HSTS 仍為 Stage 1 `max-age=300`，已觀察兩日無異常；SEC-001 現已進入 Stage 2 rollout，提升節奏見 `docs/operations/edge-security.md`。
