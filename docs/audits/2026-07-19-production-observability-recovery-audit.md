...
| 欄位 | 契約 |
| --- | --- |
| `eventSchema` | 數字 schema version；A5a 加入 snapshot window 欄位後為 `4` |
...
A5a 將「發布產品」與「記錄觀測事實」分開。D1 `snapshot_window_attempts` 保存每次 attempt，`snapshot_windows` 保存每個城市／window 的 canonical terminal outcome；`snapshot_window_completed` 則是送往短期 Logs 的 schema v4 事件。event 或 durable record 寫入失敗不得切換、回復或破壞 `dataset_versions.active_version`，但 workflow 必須以 `window-record-write-failed` 明確失敗，避免把沒有 freshness 證據的發布當成完全成功。
...
A5b 的最小介面是「在 unchanged terminal marker 前執行 active probe」：輸入 `city`、D1 actual active/previous version 與現有 R2/public clients；輸出固定 `probeResult`、artifact completeness 欄位與 allowlist failure class。A5b 可另加 migration 欄位或 probe table，再由 A5a runner 將結果併入同一 window terminal；不得讓 probe 自行另造 window identity，也不得將 route/place identity 寫入 telemetry。A5a 已提供 actual pointer read、attempt/canonical repository、strict child marker 與 terminal write 邊界。
...
- active／previous snapshot version；probe outcome 留給 A5b。
...
6. **A5b unchanged active probe**：source unchanged 也必須驗 D1 active pointer、R2 manifest、critical artifacts 與 rotating sample，結果回寫 A5a window。
7. **A6 rollback authority**：D1 為服務權威，rollback 後同步 R2 state，加入 divergence test。
8. **A7 post-deploy HTTP smoke**：release-specific pages/assets、大城小城 API、degraded contract。
9. **A8 fresh-browser smoke**：全新 context、boot marker、pageerror、unhandled rejection、asset failure。
10. **A9 frontend organic collector**：bounded payload、URL query removal、sampling、dedupe、rate limit；與 CSP route 分開。
11. **A10 daily 22-city probe**：硬狀態輸出、active/previous 與 rotating artifacts。
...
| A5a snapshot window outcome | 本機實作與 review/check 完成，待獨立提交 | D1 attempt/canonical tables、deterministic window ID、strict phase markers、實際 active pointer reconciliation、schema v4 event 與 GitHub summary；targeted 5 files/59 tests、完整 check 64 files/444 tests、typecheck、build、Worker dry-run、workflow YAML、local migration apply/reapply 通過；不含 active artifact probe 或 watchdog |
| A5b–A10 | 已排程，未開始 | A5b 獨立處理 unchanged active probe；各批保持可獨立 review/rollback |
| Phase B/C | 未開始 | 等 Phase A 事件量與操作需求證明 |

## 12. 最值得先實作的三項

1. 完成 A5a snapshot window outcome，建立每城市／window 的 durable terminal 證據。
2. A5b unchanged active probe，消除「source 未變但 active artifacts 已損壞」盲區。
3. A6 rollback authority，讓 D1 active pointer 與 R2 metadata 有單一權威與 divergence test。
