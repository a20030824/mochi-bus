...
| 欄位 | 契約 |
| --- | --- |
| `eventSchema` | 數字 schema version；A6b 加入 public probe 事件後為 `7` |
...
| `source` | `realtime | stale | schedule | snapshot | fallback | mixed | browser | worker | tdx_static | none` |
...
### 5.5 A4 TDX resolution contract

一筆 `tdx_resolution_completed` 代表「一個固定 operation 與 credential scope 的邏輯 TDX 資料取得」。memory、edge、circuit、upstream、一次受控 retry 與 stale replay 都包含在同一 resolution 內，最終最多產生一筆 completion；不得將 cache miss、每次 retry 或 stale fallback 各自當成分母事件。

第一批 operation 為 `route_catalog`、`place_arrivals`、`vehicle_positions`、`journey_eta`、`tdx_schedule`。snapshot schedule 不屬於 TDX resolution；產品最後顯示 snapshot schedule 時，只由 A3 的 `api_operation_completed` 表達 degraded／schedule 語意。城市 route catalogue 與共用的 intercity static catalogue 都使用 `route_catalog`，後者的 `city` 為 `null`。

| 最終路徑 | result | cacheResult | failure／retry 語意 |
| --- | --- | --- | --- |
| memory hit | `success` 或合法 `empty` | `memory_hit` | 無 upstream status；保留已知 data age，舊格式無時間戳時為 `unknown` |
| edge hit | `success` 或合法 `empty` | `edge_hit` | 不再記 upstream；缺少可信時間戳時不得猜 freshness |
| cache miss → upstream contract 有效 | `success` 或合法 `empty` | `miss` | upstream status class 為 `2xx` |
| timeout／network／5xx 後一次 retry 成功 | `success` 或合法 `empty` | `miss` | `retryCountBucket=1`、`recoveredAfterRetry=true`、嚴格 `initialFailureClass` |
| upstream failure → stale replay | `degraded` | `bypass` | resolution=`stale_replay`，保留最終 failure class 與 data age bucket |
| circuit open → stale replay | `degraded` | `bypass` | 沒有 upstream attempt，因此 upstream status 為 `none` |
| circuit open，無 stale | `error` | `bypass` | resolution／failure class 都為 `circuit_open` |
| local arrivals cooldown | `error` | `bypass` | resolution=`none`；不冒充 TDX 429 response |
| token rejected、429、quota、invalid JSON/schema | `error` | 依路徑 | 不受控 retry；401 與 429 不會因 telemetry 形成 retry loop |

`credentialScope` 只允許 `shared | byok | none`；事件不包含 client ID、token、credential/cache fingerprint、URL/query、route/place/stop identity、車牌、座標、raw error 或 stack。organic resolution 在開始時固定採 10% cohort，success／empty／degraded／error 使用同一決策；`synthetic` 與 `snapshot_publish` 為 100%。目前公開產品 route 仍只建立 `user` traffic；未來 smoke 必須由受信任執行路徑注入，不能信任公開 header 自稱 synthetic。

分母限制：`tdx_resolution_completed` 可計算所有 logical resolution 的 memory／edge／upstream／circuit／stale 比例，但不能精確還原每一次 upstream attempt；retry 只以 `0 | 1 | 2_plus` bucket 與 `recoveredAfterRetry` 表達。因此 upstream 健康率必須限定 `resolution=upstream`，不能拿全部 cache-inclusive resolution 當 upstream attempt 分母。

half-open probe 成功時，現有 circuit state 會被清除，測試也以後續 resolution 不再被擋住來證明恢復；但 A4 completion 不另加跨 resolution 的「circuit recovered」欄位，也不把它冒充 `recoveredAfterRetry`。因此只看 completion 無法將 half-open success 與一般 upstream success 分開；若實際資料量證明需要，再由獨立、低基數的 `circuit_state_changed` 診斷事件補足。

BYOK refresh 由 browser `web/tdx/api-client.ts` 完成：一次舊 token 401 與刷新後的新 Worker request 是兩個 HTTP request。Worker 不接觸 Client Secret，也禁止建立 credential fingerprint 或跨 request 使用者關聯，所以 A4 不會假裝將這兩個 request 合成一筆 server resolution。瀏覽器既有契約測試另行證明「只 refresh／retry 一次、第二次 401 停止、shared request 不 refresh」；Worker 事件只記每個 inbound request 的 `byok` scope 與最終結果。

### 5.6 A5a snapshot window outcome contract

A5a 將「發布產品」與「記錄觀測事實」分開。D1 `snapshot_window_attempts` 保存每次 attempt，`snapshot_windows` 保存每個城市／window 的 canonical terminal outcome；`snapshot_window_completed` 在 A5a commit 時為 schema v4，A5b 擴充 probe 後全體事件 schema 升為 v5。event 或 durable record 寫入失敗不得切換、回復或破壞 `dataset_versions.active_version`，但 workflow 必須以 `window-record-write-failed` 明確失敗，避免把沒有 freshness 證據的發布當成完全成功。

Window ID 不依賴 workflow run ID。排程 window 使用台灣時間的城市預定日與 03:17 slot：`v1:<city>:YYYY-MM-DD:0317`；GitHub rerun 先依原 run ID 找回同一 window。手動執行預設仍歸屬該城市最近的 scheduled window；只有明確選擇 `manual` window type 時才使用 `v1:<city>:YYYY-MM-DD:manual`。`force_publish` 只改發布決策，不另造 window。

Canonical terminal result 只允許 `published | unchanged | failed`。同 window 的 failed 可由較新的 attempt 更新為 unchanged／published，但已成功的 canonical outcome 不可被較舊或較低等級 attempt 降回 failed。`missing` 不由同步 workflow 寫入；未來 watchdog 只在 window close 後查不到 terminal outcome 時推導。

時間欄位保持不同語意：`lastSourceCheckAt` 是本次成功取得並判讀 source 的時間；`lastPublishedAt` 是 D1 active pointer 最後成功切換的時間；`completedAt` 是本次 attempt 的 terminal 時間；snapshot 自身 generated/imported time 仍屬 artifact version。unchanged 只更新 source check，不更新 publish time；failed 在 source 尚未成功判讀時維持前值或 `null`。terminal record 會在 child 結束後重新讀取實際 D1 pointer，因此 activate 後 finalize 失敗仍反映真正的 active version。

固定 failure class 只描述階段：`snapshot_source_fetch`、`snapshot_source_compare`、`snapshot_active_pointer_read`、`snapshot_local_validation`、`snapshot_stage`、`snapshot_remote_validation`、`snapshot_activate`、`snapshot_smoke`、`snapshot_rollback`、`snapshot_finalize`、`snapshot_window_record_write`、`unknown`；禁止 raw error、URL、credential、route/stop 或 stack。D1 查詢使用 bound parameters；GitHub summary 也只接受嚴格的低基數欄位。

A5a 已獨立提交並推送為 `aad570b`。

### 5.7 A5b unchanged active probe contract

Source hash 相同不再直接 `unchanged` return。publisher 先用 D1 `dataset_versions.active_version` 取得唯一權威版本，再完成下列 hard probes；任何一項失敗都發固定 `snapshot_active_probe` marker、使 window `failed`，保留舊 active pointer且不自動 force publish：

| Hard group | 成功條件 | Failure class |
| --- | --- | --- |
| D1 authority | active pointer 存在、格式有效 | `active_pointer_missing`／`active_pointer_invalid` |
| D1 usability | routes/patterns/stops/places/pattern stops 非零；每個 catalogue route 至少一個 pattern | `active_rows_empty`／`route_without_pattern` |
| Manifest | schema/city/version 等於 D1 active；核心 counts 一致；manifest 含 network/shape/schedule/place 類別 | `manifest_missing`／`manifest_version_mismatch`／`manifest_count_mismatch` |
| Network | R2 HEAD size 等於 manifest bytes；前 64 KiB metadata 宣告同 city/version | `network_missing`／`network_version_mismatch` |
| Public catalogue | schema/source/version 正確且 route count 等於 D1/manifest | `public_routes_failed`／`public_schema_invalid`／`public_version_mismatch`／`public_count_mismatch` |
| Rotating route | deterministic pattern 的 detail 為 snapshot，且至少兩站 | `route_sample_failed` |
| Rotating place |同一 sample 的 arrivals 明確使用 `place-bundle` 且含該 pattern | `place_bundle_sample_failed` |

診斷 probes 不把 current active 冒充 unavailable：R2 state 與 D1 分歧、previous 不完整，以及非核心 rotating shape/schedule HEAD 失敗，分別記 `state_pointer_mismatch`、`previous_unavailable`、`shape_sample_unavailable`、`schedule_sample_unavailable`。active hard checks 全過但有診斷 warning 時，probe=`degraded`、window 仍可 `unchanged`、`rollbackAvailable=false`，GitHub summary 列為 `unchanged-rollback-degraded`；不標 Red，但 rollback 工具 fail closed。

Sample selector 對穩定排序後的候選集合使用 `SHA-256(city + windowId + probeCaseVersion) % candidateCount`。同 window rerun 選相同 case，跨 window 輪替；durable state／telemetry 只保存 `case_<12 hex>`，不保存 RouteUID、routeName、pattern/place/stop ID、artifact key 或 URL。

雙北 network 不完整下載或 parse。probe 先 HEAD，再以 signed Range GET 請求 `bytes=0-65535`；即使伺服器忽略 Range，也透過 bounded stream reader 在超過 64 KiB 時取消。公開 JSON 與 manifest 也有 2 MiB／1 MiB 上限。這遵守 Workers/R2 的 streaming 原則，且不建立 Leaflet index。

D1 migration `0004_snapshot_active_probes.sql` 建立 `snapshot_probe_attempts` 與 canonical `snapshot_active_probes`，保存 probe schema/version、city/window/attempt、actual active/previous、probe time/result/failure、rollback availability、case version/ID、hard checks count 與 allowlist diagnostics。window attempt、canonical window、probe attempt、canonical probe 以同一 D1 batch 完成，避免 `unchanged` canonical 存在但 probe evidence 缺席；較新的 error attempt保留在 history，但不降級同 window 已成功的 canonical probe。

`snapshot_probe_completed` 是每次整體 active probe唯一的權威 completion：`trafficClass=publish_smoke`、sample 100%、versionRole=`active`、probeGroup=`active_snapshot`。子檢查不另造 completion 分母；event/schema/emitter failure 不改變產品資料。probe identity 與 window terminal identity、runner 完成後重讀的 actual D1 pointer 必須一致，否則 window 以 `active_pointer_invalid` 失敗。
...
`window_watchdog_completed` 每城市恰好一筆，`trafficClass=synthetic`、sample 100%，只含 city/window、固定 status/result/failure、版本、age bucket、rollback availability 與 latency bucket；不含 workflow URL、artifact key、route/place identity、credential、raw error 或 stack。A6b 的 `public_probe_completed` 另外每日從公網獨立驗證 22 城 routes/network/route/place/journey(見 `docs/operations/public-network-probe.md`)。
...
- `windowId`：城市 + 台灣預定日 + schedule slot；不使用 workflow run ID。
- `startedAt`：workflow 開始處理城市。
- `lastSourceCheckAt`：確認過 TDX source hash／資料的時間。
- `lastPublishedAt`：只有 active pointer 成功切換才更新。
- durable `result`：`published | unchanged | failed`；`missing` 由 watchdog 推導。
- active／previous snapshot version與 A5b probe outcome；watchdog/missing 留給 A6。
...
5. **A5a snapshot window outcome**：D1 durable attempts/canonical outcome、固定 window identity、source/publish 時間分離與安全 GitHub summary。
6. **A5b unchanged active probe**：source unchanged 也驗 D1 active pointer、R2 manifest/network、public catalogue 與 deterministic route/place sample，結果回寫 A5a window。
7. **A6 missed-window watchdog + daily 22-city public probe**：07:30 close 推導 missing，並輸出硬狀態、active/previous 與 rotating artifacts。
8. **A7 rollback authority**：D1 為服務權威，rollback 後同步 R2 state，加入完整 divergence/reconcile test。
9. **A8 post-deploy HTTP smoke**：release-specific pages/assets、大城小城 API、degraded contract。
10. **A9 fresh-browser + organic frontend evidence**：全新 context boot/pageerror/asset failure；之後才加 bounded organic collector，與 CSP route 分開。
...
| A6b daily public probe | 本機完成，待獨立 review/commit | 公網獨立證據鏈(GitHub → DNS/TLS → Worker → public API → active snapshot → route/place/journey)、hard health 與 realtime diagnostics 兩平面、10 hard checks、64 KiB network prefix、獨立 rotation 種子、expensive 呼叫 2.5s 配速、D1 run/attempt/canonical tables、schema v7 city completion 事件 |
| A7–A9 | 已排程，未開始 | 各批保持可獨立 review/rollback |
