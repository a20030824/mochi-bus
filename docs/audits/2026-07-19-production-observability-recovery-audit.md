...
| 欄位 | 契約 |
| --- | --- |
| `eventSchema` | 數字 schema version；A4 加入 TDX resolution 欄位後為 `3` |
...
5. **A5 snapshot window outcome + unchanged active probe**：weekly window outcome 與 07:30 close 判定；即使 source unchanged／early-exit，仍驗 D1 active pointer、R2 manifest、critical artifacts 與 rotating sample。
6. **A6 rollback authority**：D1 為服務權威，rollback 後同步 R2 state，加入 divergence test。
...
| A4 TDX resolution completion | 本機實作與 review/check 完成，待獨立提交 | schema v3、complete-once logical resolution tracker、memory/edge/upstream/circuit/stale、受控 retry、五類 TDX operation；targeted 7 files/95 tests、完整 check 60 files/414 tests、typecheck、build、Worker dry-run 通過；不改 A3 分母 |
| A5–A10 | 已排程，未開始 | A5 合併 snapshot window outcome 與 unchanged active probe；各批保持可獨立 review/rollback |
| Phase B/C | 未開始 | 等 Phase A 事件量與操作需求證明 |

## 12. 最值得先實作的三項

1. 完成 A4 TDX resolution，建立 cache-inclusive resolution 分母與 failure/retry 原因。
2. A5 snapshot window outcome + unchanged active probe，消除「同步沒發布就完全沒訊號」的盲區。
