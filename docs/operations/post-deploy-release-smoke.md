...
每次 deploy 使用新 Chromium browser context，`serviceWorkers: block`，依序開啟三個主要頁面。每個頁面的主要 shell／map boot selector 可見後，browser context 會再讀取帶 synthetic probe token、`Cache-Control: no-store` 的 release endpoint；這不是一次性抽樣，而是最多 60 秒、每 5 秒一次的 bounded propagation gate。
