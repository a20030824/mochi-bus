...
Taipei 額外使用產品正式預設 identity `307`／`TPE19108`。Runner 必須先在 catalogue 找到這一筆，再讀取 exact snapshot route variant、第一個 stop 的 canonical place，最後讀取 place arrivals。不得拿 `routes[0]` 或其他偶然排序結果當作 release smoke sample；catalogue 找不到固定 identity 回報 `route_sample_missing`，route HTTP／timeout 回報 `route_http_failed`，只有 response schema、source 或 exact variant identity 不符才回報 `route_contract_invalid`。

Arrivals 無論當下 realtime healthy 或 degraded，都必須維持：
