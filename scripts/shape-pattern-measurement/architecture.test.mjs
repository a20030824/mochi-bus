...
describe('measurement harness production isolation', () => {
  it('pins the production matcher blob without modifying its semantics', async () => {
    expect(gitBlobSha1(await readFile(MATCHER_SOURCE))).toBe(SUPPORTED_MATCHER_GIT_BLOB_SHA1)
  })

  it('is not imported by src, web, Vite, or Worker configuration', async () => {
    ...
    for (const path of paths) {
      ...
      expect(source, path).not.toContain('shape-pattern-measurement/run.mjs')
    }
  })

  it('keeps raw, generated, and report artifacts ignored', async () => {
    const gitignore = await readFile('.gitignore', 'utf8')
    expect(gitignore).toContain('/.tdx-measurement/')
  })

  it('does not add the harness to production start or build commands', async () => {
    ...
    expect(pkg.scripts['build:map']).not.toContain('shape-pattern-measurement')
    expect(pkg.scripts.dev).not.toContain('shape-pattern-measurement')
    expect(pkg.scripts.deploy).not.toContain('shape-pattern-measurement')
  })
})
